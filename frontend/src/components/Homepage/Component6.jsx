...
function Component6() {
  return (
    <>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-8 xl:gap-10 m-8">
        {cardData.map((card, index) => (
          <div
            className="card bg-base-100 shadow-xl flex flex-col h-full min-w-[200px]"
            key={index}
          >
            ...
            <div className="card-body flex-grow p-5">
              <h2 className="card-title text-l">{card.title}</h2>
              <p className="text-sm">{card.description}</p>
              <div className="card-actions justify-end">
                <button className="btn hover:bg-purple-400 hover:text-white px-4 py-2 rounded-md shadow-md transition-colors duration-300">
                  Read More
                </button>
              </div>
            </div>
